...
def main():
    ...
    # (hide by elevating logging level:
    logging.info(u"Open Allure Version: %s" % __version__)
    logging.debug(u"Pygame Version: %s" % pygame.__version__)
    ...
    logging.info(u"Question sequence Loaded with %s questions" %
                 str(len(seq.sequence)))
    ...
    try:
        ...
        url = url, q = 0)
        logging.info("Database %s opened." % oadb)
    except IOError:
        ...
        record_id = openallure.db.insert(time = time.time(), \
        url = url, q = 0)
        logging.info("Database %s created." % oadb)
                ...
                if event.key in range(pygame.K_1, pygame.K_6) and \
                    ...
                    openallure.currentString == u'')):
                        ...
                        if answer < choiceCount:
                            ...
                            logging.info("Open Allure database record %s posted" % str(record_id))
                            if graphViz:
                                graphViz.kill()
                                oagraph(seq,openallure.db,url,openallure.showText,openallure.showResponses,openallure.showLabels)
                                graphViz = subprocess.Popen([graphVizPath, 'oagraph.dot'])
                    ...
                    if openallure.currentString:
                        ...
                        if nltkType == 'graph':
                            ...
                            if nltkName == 'hideText':
                                ...
                                logging.info("Open Allure database reset")
